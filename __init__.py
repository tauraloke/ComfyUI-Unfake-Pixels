...
class PixelArtScaler:
    ...
    def runs_based_detect(self, img: Image.Image) -> int:
        """Определяет масштаб на основе длин серий пикселей."""
        logger.info("Detecting scale using runs-based method")
        img = img.convert('RGB') # Убедимся, что работаем с RGB
        np_img = np.array(img)

        all_run_lens = []

        def scan_runs(is_horizontal: bool):
            if is_horizontal:
                primary_dim, secondary_dim = np_img.shape[0], np_img.shape[1] # H, W
            else:
                primary_dim, secondary_dim = np_img.shape[1], np_img.shape[0] # W, H

            for i in range(primary_dim):
                current_run_length = 1
                prev_color = None
                for j in range(secondary_dim):
                    if is_horizontal:
                        color = tuple(np_img[i, j]) # (R, G, B)
                    else:
                        color = tuple(np_img[j, i]) # (R, G, B)

                    if prev_color is not None and color == prev_color:
                        current_run_length += 1
                    else:
                        if current_run_length > 1:
                            all_run_lens.append(current_run_length)
                        current_run_length = 1
                    prev_color = color
        ...
        scan_runs(True)  # Горизонтальные серии
        scan_runs(False) # Вертикальные серии

        if not all_run_lens:
             logger.warning("No runs found for scale detection, defaulting to 1.")
             return 1

        # Находим НОД длин серий
    ...
    def downscale_by_dominant_color(self, img: Image.Image, scale: int) -> Image.Image:
        ...
        if new_w <= 0 or new_h <= 0:
            raise ValueError(f"Scale factor {scale} is too large for image size {orig_w}x{orig_h}")

        img_array = np.array(img)
        # Добавляем измерения для векторизации
        # Форма: (new_h, scale, new_w, scale, 3)
        ...
        # Для каждого блока (i, j) находим доминирующий цвет
        downsampled_array = np.zeros((new_h, new_w, 3), dtype=np.uint8)
        for i in range(new_h):
            for j in range(new_w):
                block_pixels = block_view[i, j] # (scale*scale, 3)
                # Находим уникальные цвета и их количество
                colors_tuples = [tuple(color) for color in block_pixels]
                if colors_tuples:
                    color_counts = Counter(colors_tuples)
                    dominant_color = color_counts.most_common(1)[0][0]
                    downsampled_array[i, j] = dominant_color
                else:
                    # На случай пустого блока (маловероятно)
                    downsampled_array[i, j] = [0, 0, 0]
    ...
    def process(self, image: torch.Tensor, max_colors: int, cleanup_jaggies: bool, downscale_method: str, scale_detection_method: str):
        """
        ...
        if max_colors < 256 and initial_colors > max_colors:
             try:
                 ...
                 logger.error(f"Quantization failed: {e}. Proceeding with original image.")
        else:
        ...
        if scale > 1:
            try:
                if downscale_method == 'dominant':
                    ...
                    logger.info(f"Downscaled by {scale}x using PIL NEAREST resampling to {new_size[0]}x{new_size[1]}")
                else:
                    pil_image = self.downscale_by_dominant_color(pil_image, scale)
            except Exception as e:
                 logger.error(f"Downscaling failed: {e}. Proceeding with quantized image.")
        else:
        ...
        manifest = {
            ...
            "processing_steps": {
                "scale_detection": {
                    ...
                    "detected_scale": scale if scale_detection_method != 'none' else None,
                },
                "color_quantization": {
